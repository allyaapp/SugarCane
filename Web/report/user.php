...
$pdf->Cell(0, 5, 'Laporan Data User', '0', '1', 'L', false); //width, height, text
$pdf->Ln(7); // Line break

$pdf->SetFont('Arial','B',12); //font, style, size
$pdf->Cell(12,6, 'No.',1,0,'C'); //width, height, text
$pdf->Cell(26,6, 'ID User',1,0,'C'); //width, height, text
$pdf->Cell(50,6, 'Nama User',1,0,'C'); //width, height, text
$pdf->Cell(70,6, 'Alamat',1,0,'C'); //width, height, text
$pdf->Cell(37,6, 'No.HP',1,0,'C'); //width, height, text
$pdf->Cell(37,6, 'Username',1,0,'C'); //width, height, text
$pdf->Cell(37,6, 'Password',1,0,'C'); //width, height, text
$pdf->Ln(2); // Line break
...
while($row = mysqli_fetch_array($query)){
	...
	$pdf->Cell(12, 6, $no.".", 1, 0, 'C'); //width, height, text
	$pdf->Cell(26, 6, $row['id_user'], 1, 0, 'C'); //width, height, text
	$pdf->Cell(50, 6, $row['nama'], 1, 0, 'L'); //width, height, text
	$pdf->Cell(70, 6, $row['alamat'], 1, 0, 'L'); //width, height, text
	$pdf->Cell(37, 6, $row['no_hp'], 1, 0, 'R'); //width, height, text
	$pdf->Cell(37, 6, $row['username'], 1, 0, 'L'); //width, height, text
	$pdf->Cell(37, 6, $row['password'], 1, 0, 'L'); //width, height, text
	$pdf->Ln(2);
}
